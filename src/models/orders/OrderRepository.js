...
export default class ProductsRepository {
  ...
  async getOrdersByName(name_user) {
    try {
      const ordersOfUser = await this.pg.manyOrNone("SELECT users.id AS cliente_id,users.name AS cliente_nome, users.cnpj AS cliente_cnpj, orders.id AS pedido_id, orders.status AS pedido_status, orders.order_date AS order_date, products.id AS produto_id, products.name AS produto_nome, products.toughness AS produto_dureza, products.dimension AS produto_dimensao, products.description AS produto_desc, order_details.quantity AS produto_quantidade, order_details.unitary_price AS produto_preco, array_agg(product_images.image_url) AS produto_imagens, order_details.full_price FROM orders INNER JOIN users ON orders.client_id = users.id INNER JOIN order_details ON orders.id = order_details.order_id INNER JOIN products ON order_details.product_id = products.id LEFT JOIN product_images ON products.id = product_images.product_id WHERE users.name = $1 GROUP BY users.id, users.name, users.cnpj, orders.id, orders.status, orders.order_date, products.id, products.name, products.toughness, products.dimension, products.description, order_details.quantity, order_details.unitary_price, order_details.full_price;", [name_user]);
      return ordersOfUser;
    } catch (error) {
      throw error;
    }
